...
def test_assign_subscription_computes_due_date_from_duration_type(client, db_session, staff_user, staff_headers):
...
def test_assign_subscription_rejects_inactive_plan(client, db_session, staff_user, staff_headers):
    member = make_member(db_session, enrolled_by=staff_user.id)
...
def test_record_payment_rejects_non_positive_amount(client, db_session, staff_user, staff_headers):
    member = make_member(db_session, enrolled_by=staff_user.id)
    plan = make_plan(db_session)
    subscription = client.post(
        f"/api/v1/members/{member.id}/subscriptions", json={"plan_id": plan.id}, headers=staff_headers
    ).json()
...
def test_record_payment_advances_due_date_and_resets_status_to_active(
    ...
    updated = client.get(f"/api/v1/members/{member.id}/subscriptions", headers=staff_headers).json()[0]
...
def test_record_payment_wrong_member_for_subscription_is_rejected(
    ...
    member_a = make_member(db_session, enrolled_by=staff_user.id, full_name="A", phone="1")
    member_b = make_member(db_session, enrolled_by=staff_user.id, full_name="B", phone="2", email="b@example.com")
    plan = make_plan(db_session)
    subscription = client.post(
        f"/api/v1/members/{member_a.id}/subscriptions", json={"plan_id": plan.id}, headers=staff_headers
    ).json()
...
def test_overdue_and_expiring_soon_subscriptions_are_flagged(client, db_session, staff_user, staff_headers):
    plan = make_plan(db_session)  # monthly, +30 days

    overdue_member = make_member(db_session, enrolled_by=staff_user.id, full_name="Overdue", phone="3")
    client.post(
        f"/api/v1/members/{overdue_member.id}/subscriptions",
        json={"plan_id": plan.id, "start_date": (date.today() - timedelta(days=40)).isoformat()},
        headers=staff_headers,
    )

    expiring_member = make_member(
        db_session, enrolled_by=staff_user.id, full_name="Expiring", phone="4", email="expiring@example.com"
    )
    client.post(
        f"/api/v1/members/{expiring_member.id}/subscriptions",
        json={"plan_id": plan.id, "start_date": (date.today() - timedelta(days=27)).isoformat()},
        headers=staff_headers,
    )

    healthy_member = make_member(
        db_session, enrolled_by=staff_user.id, full_name="Healthy", phone="5", email="healthy@example.com"
    )
    client.post(
