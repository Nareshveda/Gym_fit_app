...
def test_check_in_creates_open_attendance_record(client, db_session, staff_user, staff_headers):
    member = make_member(db_session, enrolled_by=staff_user.id)

    response = client.post(
        "/api/v1/attendance/check-in", json={"member_id": member.id}, headers=staff_headers
    )
...
def test_duplicate_check_in_without_check_out_is_rejected(client, db_session, staff_user, staff_headers):
    member = make_member(db_session, enrolled_by=staff_user.id)
...
def test_check_out_closes_the_record(client, db_session, staff_user, staff_headers):
    ...
    response = client.put(f"/api/v1/attendance/{check_in['id']}/check-out", headers=staff_headers)
...
def test_double_check_out_is_rejected(client, db_session, staff_user, staff_headers):
    ...
    second = client.put(f"/api/v1/attendance/{check_in['id']}/check-out", headers=staff_headers)
    assert second.status_code == 409


def test_check_in_again_after_check_out_is_allowed(client, db_session, staff_user, staff_headers):
    member = make_member(db_session, enrolled_by=staff_user.id)
    check_in = client.post(
        "/api/v1/attendance/check-in", json={"member_id": member.id}, headers=staff_headers
    ).json()
